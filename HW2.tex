...
\hlcom{#We know this is effectively the mean of the left and right point methods}
\hldef{(trap.area} \hlkwb{=} \hlnum{0.5}\hlopt{*}\hldef{(right.area}\hlopt{+}\hldef{left.area))}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
                         ...
                         \hlkwc{method} \hldef{=} \hlsng{"Right"}\hldef{)\{}    \hlcom{# method to use (trap by default)}
  \hlcom{######################################}
  \hlcom{# Check Input}
  \hlcom{######################################}
  ...
  \hlcom{# Compute Area}
  \hlcom{######################################}
  \hldef{delta.x} \hlkwb{=} \hldef{(b}\hlopt{-}\hldef{a)}\hlopt{/}\hldef{n.rect} \hlcom{#Pre-calculating data used in several integral estimates}
  ...
  \hlcom{#Calculating left and right areas ahead of time simplifies trapezoid rule}
  \hldef{left.area} \hlkwb{=} \hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(}\hlkwd{fnct}\hldef{(left.points)))}
  \hldef{right.area} \hlkwb{=} \hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(}\hlkwd{fnct}\hldef{(right.points)))}
  ...
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Midpoint"}\hldef{)\{}
    \hlcom{# Add necessary code here}
    \hldef{mid.points} \hlkwb{=} \hldef{(left.points}\hlopt{+}\hldef{right.points)}\hlopt{/}\hlnum{2}
    \hldef{mid.area} \hlkwb{=} \hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(}\hlkwd{fnct}\hldef{(mid.points)))}
    \hldef{arae} \hlkwb{=} \hldef{mid.area}
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidial"}\hldef{)\{}
    \hlcom{# Add necessary code here}
    \hldef{trap.area} \hlkwb{=} \hlnum{0.5}\hlopt{*}\hldef{(right.area}\hlopt{+}\hldef{left.area)}
    \hldef{area} \hlkwb{=} \hldef{trap.area}
